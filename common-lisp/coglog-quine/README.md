...
| | フォン・ノイマンの自己増殖オートマトン | Recurrent Quine |
|---|---|---|
| 自己記述 | テープ上の命令列 | `*self-source*`（quoted form） |
| 構築機構 | 万能構成子 | `write-generation` |
| 複製の忠実度 | テープの読み取り精度 | `print → read` の round-trip 精度 |
| 複製の対象 | 自分と同一の構造 | 自分と異なる次世代（ $Q_n \neq Q_{n+1}$ ）|
| 環境 | セルオートマトンの格子空間 | LLM + サンドボックス（実在するインフラ） |
...
| | 従来の ALife | Recurrent Quine |
|---|---|---|
| 環境 | 設計者が作った閉じたシミュレーション | 実在するインフラ（LLM + サンドボックス） |
| 資源 | CPU 時間、メモリ（仮想） | コンテキストウィンドウ、ファイル I/O（実在） |
| 淘汰圧 | 設計者が定義した適応度関数 | Eigen の閾値（round-trip 忠実度）。適応度関数は不在 |
| 集団 | 多数の個体が空間的に共存 | 集団サイズ1。時間方向の揺らぎで探索を代替 |
| 進化 | ランダムな突然変異 + 自然選択 | 遷移関数と複製機構の意図的な書き換え経路が開かれているが、使うかどうかは LLM に委ねられている |
| 宿主依存性 | シミュレータに依存 | LLM + Common Lisp 処理系に依存。ディスク上では不活性 |
